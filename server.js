...
mongoose.connect('mongodb://localhost/youapp-chat');
...
const secretKey = 'RYkxZuhGkzvEXHLTiMJlFn1P';

app.get('/', async (request, response) => {
    response.sendFile(__dirname + '/index.html');
});

app.post('/api/register', handleFileUpload, async (request, response) => {
    try {
        if(!request.file){
...
app.get('/api/updateProfile', verifyToken, async (request, response) => {
...
app.put('/api/updateprofilesetting/:id', handleFileUpload, async (request, response) => {
    try {
...
app.post('/api/searchProfile', async (request, response) => {
...
app.post('/api/loadMessages', async (request, response) => {
    let chatRequestData = [];
    ...
    }; ///api/message>>_request_receiver_id = ${receiver_user_id} AND chat_request_status = 'pending'
...
app.put('/api/acceptRequest/:id', async (request, response) => {
...
app.post('/api/viewMessage', async (request, response) => {
...
app.listen(PORT, () => {
    console.log(`youapp-chat Application running on Port No. ${PORT}`);
});